...
This copy is pre-filled for **Rohan Mandal** / GitHub user **Rohan-14-1**: the typing banner,
socials (LinkedIn, email, portfolio, X), the "whoami" bullets, `assets/skills.json`, and
`assets/projects.json` (featuring `AI-Road-Damage-V2V`, `LoanPredict-AI`,
`Generative-AI-for-Legal-Docs`, and `job-portal`).

Optional: add a photo and generate the portrait —

```bash
python scripts/dotify.py your-photo.jpg -o assets/portrait --cols 100 --equalize --detail 0.5 --color
```

then uncomment the `<img src="assets/portrait.svg" ...>` line near the top of `README.md`.

Swap in any different projects by editing `assets/projects.json` and `assets/skills.json`, then
re-run (needs a GitHub token locally to avoid rate limits, or just let the Actions workflow do it
after you push — see step 5):

```bash
python scripts/radar.py --data assets/skills.json -o assets/radar
python scripts/cards.py --user Rohan-14-1 --projects assets/projects.json --out assets
```
